...
describe('renderer', () => {
  ...
  describe('created element', () => {
    it('has tag', () => {
      hostElm = vdom.patch(hostElm, vnode0, h('div', null)).elm;
    ...
    it('receives css classes', () => {
      ...
      hostElm = vdom.patch(hostElm, vnode0, vnode1).elm;
      expectClasses(hostElm.firstChild, ['i', 'am', 'a', 'class']);
    });
    ...
    it('should not remove duplicate css classes', () => {
     ...
     it('can create elements with text content', () => {
       hostElm = vdom.patch(hostElm, vnode0, h('div', null, 'I am a string')).elm;
  ...
  describe('patching an element', () => {
    it('does not remove classes of previous from dom if vdom does not document them', () => {
      ...
      const vnode1 = <i class={{ i: true, am: true }} />;
      hostElm = vdom.patch(hostElm, vnode0, vnode1).elm;

      expectClasses(hostElm, ['i', 'am', 'horse']);
    });

    it('changes elements classes from previous vnode', () => {
      const vnode1 = <i class={{ i: true, am: true, horse: true }} />;
      const vnode2 = <i class={{ i: true, am: true, horse: false }} />;
      vdom.patch(hostElm, vnode0, vnode1);
      hostElm = vdom.patch(hostElm, vnode1, vnode2).elm;

      expectClasses(hostElm, ['i', 'am']);
    });

    it('preserves memoized classes', () => {
      const cachedClass = { i: true, am: true, horse: false };
      const vnode1 = <i class={cachedClass} />;
      const vnode2 = <i class={cachedClass} />;
    ...
    it('removes missing classes', () => {
      const vnode1 = <i class={{ i: true, am: true, horse: true }} />;
      const vnode2 = <i class={{ i: true, am: true }} />;
    ...
    it('removes classes when class set to empty string', () => {
      const vnode1 = <i class={{ i: true, am: true, horse: true }} />;
      const vnode2 = <i class="" />;

      vdom.patch(hostElm, vnode0, vnode1);
      hostElm = vdom.patch(hostElm, vnode1, vnode2).elm;

      expectClasses(hostElm, []);
    });
  });
    ...
    expect(hostElm.childNodes[0].textContent).toEqual('Hello');
     ...
     /*describe('updating children with keys', () => {
       ...
       describe('removal of elements', () => {
         ...
         it('removes child svg elements', () => {
           ...
           const resultA = vdom.patch(hostElm, vnode0, a);
           expect(resultA.elm.childNodes.length).toEqual(2);

           const resultB = vdom.patch(hostElm, resultA, b);
       ...
       it('handles random shuffles', () => {
         ...
         for (n = 0; n < samples; ++n) {
           ...
           vnode0.elm = elm;
           elm = vdom.patch(hostElm, vnode0, vnode1).elm;
           ...
           elm = vdom.patch(hostElm, vnode1, vnode2).elm;
           for (i = 0; i < elms; ++i) {
       ...
       it('supports all null/undefined children', () => {
         ...
         hostElm = vdom.patch(hostElm, vnode2, vnode3).elm;
     ...
     describe('updating children without keys', () => {
       ...
       it('removes a single text node', () => {
         ...
         vdom.patch(hostElm, vnode1, vnode2);
